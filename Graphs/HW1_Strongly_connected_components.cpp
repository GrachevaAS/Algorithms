...
class CListGraph : public IGraph {
 public:
  ...
  int connectComponents();  // returns number of edges needed for strongly
                            // connected graph
 private:
...
void CListGraph::GetNextVertices(int vertex, std::vector<int>& vertices) const {
  for (std::list<int>::const_iterator it = adjLists[vertex].begin();
...
void CListGraph::GetPrevVertices(int vertex, std::vector<int>& vertices) const {
  for (std::list<int>::const_iterator it = adjListsReverse[vertex].begin();
       it != adjListsReverse[vertex].end(); it++) {
    vertices.push_back(*it);
  }
}
  ...
  for (int ind = order.size() - 1; ind >= 0; ind--) {
    ...
    if (!visited[i]) {
      ...
      while (!dfsStack.empty()) {
        ...
        for (int j = 0; j < tempArr.size(); j++) {
          if (!visited[tempArr[j]]) dfsStack.push(tempArr[j]);
        }
      }
      componentsCount++;
    }
...
int CListGraph::connectComponents() {
  kosaraju();
  if (componentsCount < 2) return 0;
  CMatrixGraph* componentsGraph = new CMatrixGraph(componentsCount);
  for (int i = 0; i < vertexCount; i++) {
    for (std::list<int>::const_iterator it = adjLists[i].begin();
         it != adjLists[i].end(); it++) {
      int j = (*it);
...
void CMatrixGraph::AddEdge(int from, int to) { Matrix[from][to] = true; }
